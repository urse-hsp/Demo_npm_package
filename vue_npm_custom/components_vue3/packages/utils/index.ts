import type { Awaits, ReqFn, Returns, Combine, Configs } from './data'
...
const Awaitsformat = (awaits: Awaits) =>
  awaits.map((obj) => (isFn(obj) ? formatter(obj) : obj))

export const combineAsyncError: Combine = (awaits: Awaits, configs: Configs) => {
  ...
  const pushResult = (value: unknown, flag = true) =>
    combine.result.push({
      flag,
      data: {
        info: value,
  ...
  new Promise<Returns>(handle).then((data) => configs.over && configs.over(data))
}
